...
const App: React.FC = () => {
  ...
  const [schemaProperties, setSchemaProperties] = useState<any>(null);
  ...
  const loadSchemaFromFile = async () => {
    ...
    try {
      const schemaPath = '/Users/sachinkumar/Documents/config-ninja/src/config/tenant-config.schema.json';
      const response = await fetch(`http://localhost:8002/api/read-file?path=${encodeURIComponent(schemaPath)}`);
  ...
  const handleSchemaSave = async (schema: any) => {
    try {
      ...
      // Reload the schema from file to reflect changes
  ...
  const renderSchemaBasedField = (fieldName: string, fieldSchema: any, value: any, parentPath: string = '') => {
      ...
      case 'boolean':
        return (
          <ToggleSwitch
            ...
            checked={value || false}
            onChange={(checked) => updateConfig(fullPath, checked)}
          />
        );
      ...
      case 'number':
        return (
          <NumberInput
            ...
            description={fieldSchema.description}
            value={value || 0}
            onChange={(val) => updateConfig(fullPath, val)}
        ...
        if (fieldSchema.items?.enum) {
          ...
          return (
            <MultiSelect
              ...
              selected={value || []}
              onChange={(selected) => updateConfig(fullPath, selected)}
            />
          );
        } else if (fieldSchema.items?.type === 'object') {
          // Handle array of objects (like tenants)
          return (
              ...
              <div className="array-items">
                {(value || []).map((item: any, index: number) => (
                  <div key={index} className="array-item">
                    <h4>Item {index + 1}</h4>
                    {fieldSchema.items?.properties && Object.entries(fieldSchema.items.properties).map(([subFieldName, subFieldSchema]) => (
                      renderSchemaBasedField(
                        subFieldName,
                        subFieldSchema,
                        item[subFieldName],
                        `${fullPath}[${index}]`
  ...
  const generateConfigFromSchema = (schema: any): TenantConfig => {
    // Generate default config based on schema
    ...
    if (schema.properties) {
      Object.entries(schema.properties).forEach(([fieldName, fieldSchema]) => {
        ...
        switch (field.type) {
          ...
          case 'integer':
          case 'number':
            defaultConfig[fieldName] = field.minimum || 0;
            break;
          case 'array':
          ...
          case 'object':
            defaultConfig[fieldName] = {};
            if (field.properties) {
              Object.entries(field.properties).forEach(([subFieldName, subFieldSchema]) => {
                const subField = subFieldSchema as any;
                if (subField.type === 'boolean') {
                  defaultConfig[fieldName][subFieldName] = false;
                } else if (subField.type === 'array') {
                  defaultConfig[fieldName][subFieldName] = [];
    ...
    return defaultConfig as TenantConfig;
  };

  // Update config to use schema-based default when schema loads
  useEffect(() => {
    if (schemaProperties && !selectedTenantFile) {
      const schemaBasedConfig = generateConfigFromSchema(schemaProperties);
      setConfig(schemaBasedConfig);
      setOriginalConfig(schemaBasedConfig);
    }
  }, [schemaProperties, selectedTenantFile]);
